...
export default function FarmDiary() {
  ...
  const handleImageUpload = (e) => {
    ...
    files.forEach(file => {
      if (file.size > 5 * 1024 * 1024) {
        alert(`${file.name}은(는) 5MB를 초과합니다. 더 작은 이미지를 선택해주세요.`);
        return;
      }
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      ...
      setEntries([...entries, { ...entryData, id: Date.now() }]);
    }
  ...
  const handleSalesSubmit = () => {
    ...
    if (!finalDate || !formData.crop || !formData.salesLocation || !formData.salesAmount || !formData.salesBoxes) {
      alert('날짜, 작물, 판매처, 판매금, 박스 수량을 모두 입력해주세요!');
      return;
    }
  ...
  const filteredEntries = entries
    .filter(entry => {
      ...
      return (
        entry.crop.toLowerCase().includes(searchLower) ||
        entry.workTypes.some(wt => wt.toLowerCase().includes(searchLower)) ||
  ...
  const pollinationEntries = entries.filter(e => e.workTypes.includes('인공수정'));
  const pesticideEntries = entries.filter(e => e.workTypes.includes('병해충 방제'));
  ...
  const getEntriesByDate = (year, month, day, workType) => {
    ...
    return entries.filter(e => e.date === dateStr && e.workTypes.includes(workType));
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            ...
            <button
              onClick={() => setCurrentView('pollination')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${
            ...
            >
              ...
              수정 타임라인
              {pollinationEntries.length > 0 && (
                <span className="bg-white text-green-600 px-2 py-0.5 rounded-full text-xs font-bold">
                  {pollinationEntries.length}
                </span>
              )}
            </button>
            <button
              onClick={() => setCurrentView('pesticide')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${
        ...
        {showForm && (
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-4 mb-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      checked={formData.useAutoDate}
                      onChange={() => setFormData({...formData, useAutoDate: true})}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      checked={!formData.useAutoDate}
                      onChange={() => setFormData({...formData, useAutoDate: false})}
                      className="w-4 h-4 text-green-600"
                    />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  작업 종류 <span className="text-red-500">*</span> (다중 선택 가능)
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  구역 {formData.workTypes.includes('인공수정') && <span className="text-red-500">*</span>} (다중 선택 가능)
                </label>
                <div className="border border-gray-300 rounded-lg p-3 bg-gray-50">
                  ...
                  {formData.areas.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
                      ...
                      {formData.areas.map(area => (
                        <span
                          key={area}
                          className="inline-flex items-center gap-1 bg-purple-500 text-white px-3 py-1 rounded-full text-sm font-medium"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({...formData, salesLocation: e.target.value})}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({...formData, salesBoxes: e.target.value})}
                      placeholder="예: 30"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-sm"
                    />
                  </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="오늘 한 작업을 자세히 기록하세요..."
                  rows="4"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                />
              </div>
            ...
            {showSalesForm && (
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      작물 <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={formData.crop}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        판매금 (원) <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        value={formData.salesAmount}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        박스 수량 <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        value={formData.salesBoxes}
                  ...
                  {formData.salesAmount && formData.salesBoxes && (
                    <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-3">
                      ...
                      <div className="text-2xl font-bold text-blue-700">
                        {(parseInt(formData.salesAmount) / parseInt(formData.salesBoxes)).toLocaleString(undefined, {maximumFractionDigits: 0})}원/박스
                      </div>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xl font-bold text-blue-700">
                          {salesStats.stats[location].totalAmount.toLocaleString()}원
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">박스:</span>
                        <span className="text-lg font-bold text-blue-600">
                          {salesStats.stats[location].totalBoxes}박스
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                  <div>
                    ...
                    <div className="text-3xl font-bold text-green-700">
                      {salesStats.grandTotalAmount.toLocaleString()}원
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600 mb-1">총 박스 수량</div>
                    <div className="text-3xl font-bold text-green-700">
                      {salesStats.grandTotalBoxes}박스
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600 mb-1">박스당 평균 단가</div>
                    <div className="text-3xl font-bold text-green-700">
                      {salesStats.averagePrice.toLocaleString(undefined, {maximumFractionDigits: 0})}원
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              {salesEntries.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  아직 판매 기록이 없습니다. 일지 작성 시 판매 정보를 입력해보세요!
                </div>
              ) : (
                      ...
                      <div className="flex gap-6 text-sm">
                        ...
                        <div>
                          ...
                          <span className="font-bold text-purple-700">
                            {(parseInt(entry.salesAmount) / parseInt(entry.salesBoxes)).toLocaleString(undefined, {maximumFractionDigits: 0})}원/박스
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        {currentView === 'pollination' && (
          <div className="bg-white rounded-lg shadow-lg p-6 overflow-x-auto">
            ...
            <div className="min-w-max">
              <div className="flex mb-2">
                ...
                {Array.from({length: 31}, (_, i) => i + 1).map(day => (
                  <div key={day} className="w-8 text-xs font-bold text-gray-700 text-center">
                    {day}
                  </div>
                ))}
              </div>
              ...
              {monthNames.map((month, monthIndex) => (
                <div key={month} className="flex border-b hover:bg-gray-50">
                  <div className="w-16 py-2 font-bold text-sm text-gray-700 flex items-center justify-center border-r-2">
                    {month}
                  </div>
                  {Array.from({length: 31}, (_, dayIndex) => {
                    ...
                    return (
                      <div
                        key={day}
                        className={`w-8 h-10 flex items-center justify-center text-xs relative ${
                          ...
                          <div className="absolute inset-0 flex flex-col gap-0.5 p-0.5">
                            {pollinations.map((poll, idx) => {
                              ...
                              return uniqueAreas.map((area, areaIdx) => (
                                <div
                                  key={`${idx}-${areaIdx}`}
                                  className={`h-1.5 ${areaColors[area]} rounded-sm`}
                                  title={`${poll.date} - ${area} - ${poll.crop}`}
                                ></div>
                              ));
                            })}
                          </div>
            ...
            <div className="mt-4 text-sm text-gray-600">
              💡 각 날짜의 색상 막대는 인공수정 작업을 나타냅니다. 마우스를 올리면 상세 정보를 볼 수 있습니다.
            </div>
          </div>
        )}
        ...
        {currentView === 'pesticide' && (
          <div className="bg-white rounded-lg shadow-lg p-6 overflow-x-auto">
                            ...
                            {pesticides.map((pest, idx) => {
                              ...
                              return uniqueAreas.map((area, areaIdx) => (
                                <div
                                  key={`${idx}-${areaIdx}`}
                                  className={`h-1.5 ${areaColors[area]} rounded-sm`}
                                  title={`${pest.date} - ${area} - ${pest.crop}`}
                                ></div>
                              ));
                            })}
                          </div>
            ...
            <div className="mt-4 text-sm text-gray-600">
              💡 각 날짜의 색상 막대는 병해충 방제 작업을 나타냅니다. 마우스를 올리면 상세 정보를 볼 수 있습니다.
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            {filteredEntries.length === 0 ? (
              <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
                {searchTerm ? '검색 결과가 없습니다.' : '아직 작성된 일지가 없습니다. 새 일지를 작성해보세요!'}
              </div>
            ) : (
                      ...
                      <div className="flex gap-2 mb-3 flex-wrap">
                        ...
                        {entry.salesLocation && (
                          <span className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                            ...
                            {entry.salesLocation} {parseInt(entry.salesAmount).toLocaleString()}원
                          </span>
                        )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3">
                      {entry.images.map(img => (
                        <img
                          ...
                          src={img.data}
                          alt={img.name}
                          className="w-full h-32 object-cover rounded-lg border-2 border-gray-200 cursor-pointer hover:border-green-500 transition"
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
              ...
              <div className="bg-pink-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-pink-700">
                  {entries.filter(e => e.workTypes.includes('인공수정')).length}
                </div>
                <div className="text-sm text-gray-600">인공수정</div>
              </div>
              <div className="bg-red-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-red-700">
                  {entries.filter(e => e.workTypes.includes('병해충 방제')).length}
                </div>
                <div className="text-sm text-gray-600">방제</div>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-purple-700">
                  {entries.filter(e => e.workTypes.includes('수확')).length}
                </div>
